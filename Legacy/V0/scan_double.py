...
scanner_id = os.environ.get("SCANNER_ID", None)
if scanner_id is None:
    print(f"[{scanner_id}] No SCANNER_ID provided.")
    sys.exit(1)

# Use scanimage to list available scanners
result = subprocess.run(["scanimage", "-L"], capture_output=True, text=True)
lines = result.stdout.strip().splitlines()
scanner_lines = [line for line in lines if "Perfection V39" in line]
...
try:
    ...
    with open(output_file, "wb") as f:
        result = subprocess.run([
            ...
            "--mode", "Color",
            "--source", "Flatbed"
            #"--buffer-size", "65536"  # ← ADD THIS LINE
        ], stdout=f, check=True)
